    ...
    for artwork in art_read_file:
        ...
        for nat in nat_list:
            if counter.get(nat) is None:
                with open(f'/Users/liwen/documents/Github/PFCH/week_4/res/{nat}.csv', 'w') as nat_file:
                    writer = csv.DictWriter(nat_file, fieldnames=art_read_file.fieldnames)
                    writer.writeheader()
                    writer.writerow(artwork)
                    counter[nat] = True
            else:
                with open(f'/Users/liwen/documents/Github/PFCH/week_4/res/{nat}.csv', 'a') as nat_file:
                    writer = csv.DictWriter(nat_file, fieldnames=art_read_file.fieldnames)
                    writer.writerow(artwork)